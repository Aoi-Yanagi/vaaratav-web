...
export default function GuestChat({ params }: { params: Promise<{ roomId: string }> }) {
  ...
  if (token === "") {
      return (
          <div className="flex flex-col items-center justify-center w-full min-h-[100dvh] bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white transition-colors duration-500">
              <Loader2 className="w-10 h-10 animate-spin text-indigo-500 mb-4" />
              <p className="text-zinc-500 dark:text-zinc-400 font-medium">Securing encrypted connection...</p>
          </div>
      );
  }
  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="flex flex-col items-center w-full h-full text-zinc-900 dark:text-zinc-100 font-sans relative"
      >
        <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-indigo-500/10 dark:bg-indigo-600/20 rounded-full blur-[120px] mix-blend-screen pointer-events-none" />
        <div className="absolute bottom-[-20%] right-[-10%] w-[600px] h-[600px] bg-cyan-500/10 dark:bg-cyan-600/20 rounded-full blur-[120px] mix-blend-screen pointer-events-none" />
        ...
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 pointer-events-none z-[90]">
            <AnimatePresence>
                {reactions.map((r) => (
                    ...
                    >
                        <div className="bg-white/80 dark:bg-black/60 backdrop-blur-sm text-zinc-900 dark:text-white text-[10px] px-2 py-0.5 rounded-full mb-1 border border-zinc-200 dark:border-white/10 transition-colors">{r.name}</div>
        ...
        <header className="flex items-center justify-between w-full max-w-5xl mt-6 px-4 z-20 shrink-0">
          <div className="flex items-center gap-3 px-5 py-2.5 bg-white/80 dark:bg-white/5 border border-zinc-200 dark:border-white/10 backdrop-blur-xl rounded-full shadow-sm dark:shadow-lg transition-colors">
            <ShieldCheck className="w-5 h-5 text-indigo-500 dark:text-indigo-400" />
            <span className="text-sm font-medium tracking-wide text-zinc-800 dark:text-zinc-200 hidden sm:block">Secure Guest Session</span>
          </div>
          
          <div className={`flex items-center gap-2 text-lg font-semibold px-5 py-2.5 rounded-full border backdrop-blur-xl shadow-sm dark:shadow-lg transition-colors ${timeLeft < 60 ? 'bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/30 text-red-500 dark:text-red-400' : 'bg-white/80 dark:bg-white/5 border-zinc-200 dark:border-white/10 text-zinc-800 dark:text-zinc-200'}`}>
            <Timer className={`w-5 h-5 ${timeLeft < 60 ? 'animate-pulse' : ''}`} />
            {formatTime(timeLeft)}
          </div>

          <div className="flex items-center gap-2">
              <Button variant="ghost" onClick={handleReportAbuse} className="group px-4 py-2.5 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 border border-red-200 dark:border-red-500/20 text-red-500 dark:text-red-400 rounded-full transition-colors">
                  <AlertTriangle className="w-4 h-4 sm:mr-2" /> <span className="hidden sm:block">Report</span>
              </Button>
              <Button variant="ghost" onClick={endSession} className="group px-5 py-2.5 bg-zinc-100 dark:bg-white/5 hover:bg-zinc-200 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-white/10 text-zinc-700 dark:text-zinc-300 rounded-full transition-colors">
        ...
        <div className="flex-1 w-full max-w-7xl p-4 sm:p-6 z-10 min-h-0 flex flex-col pb-32 sm:pb-24">
          <div className="flex-1 w-full bg-white/50 dark:bg-zinc-900/50 backdrop-blur-sm rounded-[2rem] border border-zinc-200 dark:border-white/10 overflow-hidden shadow-xl dark:shadow-2xl relative transition-colors">
